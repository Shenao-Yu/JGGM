...
def distance_w(gwr, gws):
    gwr = gwr.T
    gws = gws.T
    dis_weight = torch.mean(
        1 - torch.sum(gwr * gws, dim=-1) / (torch.norm(gwr, dim=-1) * torch.norm(gws, dim=-1) + 0.000002))
    dis = dis_weight
    return dis
...
def cluster_initialize(data, num_syn_dict, model_name, architecture, opt_parameter, file_path):
    ...
    if os.path.exists(file_path):  # load pre-calculated results
        ...
        for node_type, h in h_dict.items():
            ...
            cluster_dict[node_type] = torch.LongTensor(cluster_labels)
...
class GraphSynthesizer(nn.Module):
    ...
    def __init__(self, data, cond_rate, feat_init='cluster', edge_hidden_channels=None):
        ...
        if feat_init == 'cluster':
            ...
            x_initial_dict, indices_dict, y_syn, mask_syn = cluster_initialize(data, self.num_syn_dict, model_name,
                                                                               architecture, opt_parameter, file_path)
...
# %%
def evalue_JGGM(num_evalue, data, x_syn_dict, adj_t_syn_dict, y_syn, mask_syn,
...
def JGGM(data, cond_rate, feat_init, para_init, basicmodel, model_architecture, cond_train):
    from collections import deque
    ...
    basic_model = get_GNN(basicmodel)(**model_architecture,
                                      ...
                                      node_types=node_types,
                                      edge_types=edge_types,
                                      target_node_type=target_node_type).to(y.device)
    lazy_initialize(basic_model, x_dict, adj_t_dict)
    # if para_init == 'classaware':
    #     para_initializer = ClassAwareInitializer(basic_model, num_classes=num_classes)
    ...
    optimizer_cond = torch.optim.Adam(graphsyner.x_parameters() + graphsyner.adj_parameters(), lr=cond_train['lr'])
    ...
    for initial_i in tqdm(range(cond_train['epochs_initial']), desc='Condensation', ncols=80):
        # if para_init == 'classaware':
        #     class_id = initial_i % num_classes
        #     basic_model.reset_parameters(para_initializer.next_init(class_id))
        # else:
        ...
        for step_fine in range(cond_train['epochs_fine']):
            ...
            loss_cos = match_loss(gw_syn, gw_real)
            ...
            if step_fine < cond_train['epochs_fine'] - 1:
                ...
                train_model(basic_model, cond_train, optimizer_basic_model, x_syn_detach, adj_t_detach, y_syn, mask_syn)
        ...
        for step_coarse in range(cond_train['epochs_coarse']):
            basic_model.eval()
            ...
            if len(grad_real_window) == k:
                g_real_agg = [
                    sum(alpha_weights[i] * grad_real_window[i][j] for i in range(k))
                    for j in range(len(parameters))
                ]

                loss = match_loss(gw_syn, g_real_agg)
                optimizer_cond.zero_grad()
                loss.backward()
                optimizer_cond.step()
